...
//
// Only the Intel 8086:51ED xHCI controller will be selected.
//
// On the ASUS TUF Gaming F15 being tested:
//
//     8086:461E = other xHCI controller
//     8086:51ED = xHCI controller associated with the USB-A
//                  root hub where the test USB device appears
//
// This is intentionally hardcoded for debugging and should be
// removed later when multi-controller xHCI enumeration is added.
//
// ============================================================
...
fn read_bar0(bus: u8, device: u8, function: u8) -> Option<u64> {
    let low = read_u32(bus, device, function, PCI_BAR0);
    ...
    if memory_type == PCI_BAR_MEMORY_TYPE_64 {
        let high = read_u32(bus, device, function, PCI_BAR0 + 4);
...
fn read_bar0_size(bus: u8, device: u8, function: u8) -> Option<u64> {
    let original_low = read_u32(bus, device, function, PCI_BAR0);
    ...
    let original_high = if is_64_bit {
        read_u32(bus, device, function, PCI_BAR0 + 4)
    } else {
        0
    };
    ...
    write_u32(bus, device, function, PCI_BAR0, 0xFFFF_FFFF);

    let size_low = read_u32(bus, device, function, PCI_BAR0);
    ...
    let size_high = if is_64_bit {
        write_u32(bus, device, function, PCI_BAR0 + 4, 0xFFFF_FFFF);

        let value = read_u32(bus, device, function, PCI_BAR0 + 4);

        // Restore high half immediately.
        write_u32(bus, device, function, PCI_BAR0 + 4, original_high);
    ...
    write_u32(bus, device, function, PCI_BAR0, original_low);
...
pub fn setup_msi(device: &XhciPciDevice, vector: u8) {
    let Some(offset) = device.msi_offset else {
        if device.msix_offset.is_some() {
            serial::write_str("PCI: xHCI has MSI-X but MSI is unavailable\n");
        } else {
    ...
    let mut control = read_u16(device.bus, device.device, device.function, offset + 0x02);
    ...
    let data_offset = if control & MSI_CONTROL_64BIT != 0 {
        ...
        write_u32(device.bus, device.device, device.function, offset + 0x08, 0);
    ...
    let verified = read_u16(device.bus, device.device, device.function, offset + 0x02);
...
pub fn find_xhci() -> Option<XhciPciDevice> {
    ...
    serial::write_str(
        "PCI: DEBUGGING MODE: 8086:461E and all other xHCI controllers will be skipped\n",
    );
    ...
    for bus in 0..=255u16 {
        for device in 0..32u8 {
            ...
            for function_index in 0..function_count {
                ...
                // ------------------------------------------------
                // DEBUGGING MODE FILTER
                // ------------------------------------------------
                //
                // Only allow Intel 8086:51ED.
                //
                // Every other xHCI controller is deliberately
                    ...
                    && (vendor_id != DEBUG_XHCI_VENDOR_ID || device_id != DEBUG_XHCI_DEVICE_ID)
                {
                    ...
                    serial::write_str("PCI: DEBUGGING MODE: required vendor=");

                    serial::write_hex(DEBUG_XHCI_VENDOR_ID as u64);

                    serial::write_str(" device=");

                    serial::write_hex(DEBUG_XHCI_DEVICE_ID as u64);
                ...
                serial::write_str(
                    "PCI: DEBUGGING MODE: selected Intel 8086:51ED xHCI controller\n",
                );

                serial::write_str("PCI: DEBUGGING MODE: selected BDF=");
...
pub fn debug_dump_ports(mmio_base: usize) {
    ...
    let cap_length = unsafe { core::ptr::read_volatile((mmio_base + 0x00) as *const u8) } as usize;
    ...
    let hccparams1 = unsafe { core::ptr::read_volatile((mmio_base + 0x10) as *const u32) };

    let supports_64bit = (hccparams1 & (1 << 0)) != 0;

    let context_size_64 = (hccparams1 & (1 << 2)) != 0;

    let xecp = ((hccparams1 >> 16) as usize) << 2;

    // --------------------------------------------------------
    // Operational registers
    // --------------------------------------------------------

    let usbcmd = unsafe { core::ptr::read_volatile((op_base + 0x00) as *const u32) };

    let usbsts = unsafe { core::ptr::read_volatile((op_base + 0x04) as *const u32) };

    let pagesize = unsafe { core::ptr::read_volatile((op_base + 0x08) as *const u32) };
    ...
    for port_index in 0..max_ports {
        ...
        let portsc = unsafe { core::ptr::read_volatile(address as *const u32) };

        let portpmsc = unsafe { core::ptr::read_volatile((address + 0x04) as *const u32) };

        let portli = unsafe { core::ptr::read_volatile((address + 0x08) as *const u32) };

        let porthlpmc = unsafe { core::ptr::read_volatile((address + 0x0C) as *const u32) };
        ...
        serial::write_usize(pec as usize);

        serial::write_str(" PRC=");

        serial::write_usize(prc as usize);
