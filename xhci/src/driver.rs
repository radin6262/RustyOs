...
pub struct XhciController {
    ...
    /// HID mouse state, if a mouse is found.
...
/// Mouse-specific state bundled together.
struct MouseInfo {
    /// Slot ID of the mouse device.
    slot_id: u8,
    ...
    /// Physical address of the report buffer.
    report_buf_phys: u64,

    /// Mouse event state.
    state: MouseState,

    /// Whether an interrupt transfer is currently queued.
    transfer_pending: bool,
}
...
impl XhciController {
    ...
    fn initialize_device(&mut self, slot_id: u8, port: u8, speed: UsbSpeed) {
        ...
        let mouse_info = parsed_config.find_hid_mouse()
            .map(|(iface_num, ep)| (iface_num, ep.clone()));
        ...
        if let Some((iface_num, ref ep_desc)) = mouse_info {
            log::info!(
                "xhci: setting up HID mouse on slot={} interface={}",
                slot_id,
                iface_num
            );
    ...
    /// Set up a HID boot-protocol mouse.
    ///
    /// Sends:
    /// - SET_PROTOCOL(BOOT)
    /// - SET_IDLE(0)
    ///
    /// Then starts the interrupt IN transfer.
    fn setup_mouse(
        &mut self,
        slot_id: u8,
        iface_num: u8,
        ...
        log::info!(
        "xhci: mouse setup: slot={} iface={} ep_addr={:#x} dci={} max_pkt={}",
        slot_id,
        iface_num,
        ...
        log::debug!(
        "xhci: mouse SET_PROTOCOL(Boot), interface={}",
        iface_num
    );
                ...
                .as_mut()
        {
            ...
            self.db.ring_endpoint(
                slot_id,
                1,
            );

            match self.wait_transfer_event(
                slot_id,
            ) {
                Some(evt)
                if evt.completion_code()
                    == TRB_COMPLETION_SUCCESS =>
                    {
                        log::info!(
                    "xhci: mouse SET_PROTOCOL(Boot) succeeded"
                );
                    }

                Some(evt) => {
                    log::warn!(
                    "xhci: mouse SET_PROTOCOL(Boot) failed: code={}",
                    evt.completion_code()
                );
                }
                ...
                None => {
                    log::warn!(
                    "xhci: mouse SET_PROTOCOL(Boot) timeout"
                );
                }
            }
        }
        ...
        log::debug!(
        "xhci: mouse SET_IDLE(0), interface={}",
        iface_num
    );
                ...
                .as_mut()
        {
            ...
            match self.wait_transfer_event(
                ...
                if evt.completion_code()
                    ...
                    {
                        log::debug!(
                    "xhci: mouse SET_IDLE succeeded"
                );
                    }

                Some(evt) => {
                    log::debug!(
                    "xhci: mouse SET_IDLE returned code={} (continuing)",
                    evt.completion_code()
                );
                }
                ...
                None => {
                    log::debug!(
                    "xhci: mouse SET_IDLE timeout (continuing)"
                );
                }
            }
        }
        ...
        //     byte 2 = Y
        //
        // We request 4 bytes so mice which provide an optional
        // wheel byte are supported too.
        // --------------------------------------------------------

        const MOUSE_REPORT_SIZE:
        usize = 4;
        ...
        {
            ring.enqueue_interrupt_in(
                report_phys,
                MOUSE_REPORT_SIZE as u32,
            );
            ...
            log::debug!(
            "xhci: first mouse interrupt transfer queued"
        );
        } else {
            log::warn!(
            "xhci: mouse endpoint DCI={} has no transfer ring",
            dci
        );
            return;
        }
    ...
    fn process_hid_events(
        ...
        while let Some(evt) =
            self.evt_ring.dequeue()
        {
            // Update ERDP.
            self.rt.set_erdp(
                0,
                self.evt_ring.dequeue_phys()
                    | (1 << 3),
            );
                ...
                == TRB_TYPE_TRANSFER_EVENT
            {
                ...
                if let Some((
                        ...
                        && evt_ep == kb_dci
                    {
                            ...
                            == TRB_COMPLETION_SHORT_PACKET
                        {
                            ...
                            log::warn!(
                            "xhci: keyboard transfer error: code={}",
                            code
                        );
                        }

                        // Re-queue keyboard transfer.
                        if let Some(ring) =
                            self.transfer_rings
                                [kb_slot as usize]
                                .get_mut(
                ...
                if let Some((
                                ...
                                mouse_report_va,
                                mouse_report_phys,
                            )) = self.mouse.as_ref().map(
                    |mouse| {
                        (
                            mouse.slot_id,
                            mouse.dci,
                            mouse.report_buf_va,
                            mouse.report_buf_phys,
                        )
                    },
                ) {
                    if evt_slot == mouse_slot
                        && evt_ep == mouse_dci
                    {
                        handled = true;

                        log::trace!(
                        "xhci: mouse transfer event: code={} residual={}",
                        code,
                        evt.transfer_length()
                    );
                            ...
                            == TRB_COMPLETION_SHORT_PACKET
                        {
                            ...
                            log::warn!(
                            "xhci: mouse transfer error: code={}",
                            code
                        );
                        }

                        // Re-queue mouse transfer.
                        if let Some(ring) =
                            self.transfer_rings
                                [mouse_slot as usize]
                                .get_mut(
                ...
                if !handled {
                    log::trace!(
                    "xhci: unhandled transfer event: slot={} ep={} code={}",
                    evt_slot,
                    evt_ep,
                    code
                );
                ...
                == TRB_TYPE_PORT_STATUS_CHANGE
            {
                ...
                log::info!(
                "xhci: port status change event: port={}",
                port_id
            );
            }
    ...
    pub fn poll_keyboard(
        ...
        // Return already-buffered event first.
        if let Some(event) =
            self.keyboard
                .as_mut()
                .and_then(|kb| {
        ...
        // One event-ring drain handles both keyboard
        // and mouse transfers.
    ...
    pub fn poll_mouse(
        ...
        if let Some(event) =
            self.mouse
                .as_mut()
                .and_then(|mouse| {
                    mouse.state.next_event()
                })
        {
            return Some(event);
        }

        // Drain the same shared event ring.
        self.process_hid_events();
