...
impl InterfaceDescriptor {
    ...
    /// Is this a HID boot protocol mouse interface?
    ///
    /// class    = 3  (HID)
...
impl ParsedConfiguration {
    ...
    /// Find the first HID boot mouse interface.
    ///
    /// Returns:
    ///     (interface_number, interrupt IN endpoint)
    pub fn find_hid_mouse(
        &self,
    ) -> Option<(u8, &EndpointDescriptor)> {
        for iface in &self.interfaces {
            if iface.is_hid_mouse() {
                log::info!(
                "xhci: found HID mouse on interface {}",
                iface.b_interface_number
            );
                ...
                in &self.endpoints
                {
                    ...
                    {
                        log::info!(
                        "xhci: mouse interrupt IN endpoint: addr={:#x} max_pkt={} interval={}",
                        ep.b_endpoint_address,
                        ep.w_max_packet_size,
                        ep.b_interval,
                    );
                ...
                log::warn!(
                "xhci: HID mouse interface {} has no interrupt IN endpoint",
                iface.b_interface_number
            );
            }
        }
